...
Proceso FuncionsCorreccio
    ...
    Definir resultat Como Real;                // Real per controlar el resutlat que es rebi de cada funció
    ...
    Para posicio <- 0 hasta (qtatFuncions - 1) Hacer
        ...
        // Funcion resutlat <- operacio(numero1,numero2,operacio)
        // Funcion distancia <- obteDistancia(x1,x2,y1,y2)
        // Funcion mitjanaAritmerica <- obteMitArit(vector, midaDelVector)
    FinPara
    ...
    Si (vCorregir[0] = 1) Entonces
        pintaCapsalera("COMPROVACIO funcio operacio - PART 1/2");
        pintaTitol("Prerequisits : Cap dels 2 numeros llegits es Zero!");
        Repetir
    ...
    Si (vCorregir[1] = 1) Entonces
        pintaCapsalera("COMPROVACIO funcio operacio - PART 2/2");
        pintaTitol("Prerequisits : Nomes el segon dels numeros llegits es Zero!");
        vValorsEntrats[1] <- 0;
        resultat <- operacio(vValorsEntrats[0], vValorsEntrats[1], '/');
        Escribir vValorsEntrats[0], " / " , vValorsEntrats[1] , " = " , resultat;
    ...
    Si (vCorregir[2] = 1) Entonces
        ...
        Escribir "La distancia es : ", resultat;
    ...
    SiNo
        ...
        vNotes[5] <- 0;
    FinSi
    // FINAL comprobació de la funció obteDistancia
	
    funcioEntreControls(vNotes,matTextExercicis,6,TOTAL_NOTES);
	
    // INICI comprobació de la funció obteMitArit
    Si (vCorregir[2] = 1) Entonces
    ...
    funcioEntreControls(vNotes,matTextExercicis,7,TOTAL_NOTES);
	
    // FINAL comprobació de la funció obteMitArit
FinAlgoritmo
...
SubProceso funcioEntreControls(vNotesRebut,matTextExercicisRebuda,qtatNotesAMostrar,qtatTotalDeNotes)
    mostraResultats(vNotesRebut,matTextExercicisRebuda,qtatNotesAMostrar,qtatTotalDeNotes);
    pausa();
    netejaPantalla();
FinSubProceso
...
Funcion nota <- puntuar(notes Por Referencia, posicioNotes Por Referencia, codiExercici, vValorsEntrats, resultat, matTextExercicisRebuda)
    ...
    Definir cadenaCorreccio, textAuxiliar Como Cadena;
    cadenaCorreccio <- "Sera correcte si ";
    operador <- "X";
    nota <- 0;
        ...
        Segun codiExercici Hacer
            ...
            5: //"divisioZero": // divisio entre zero = 5
                ...
                Si (vValorsEntrats[1]=0) Entonces
                    ...
                    nota <- 2;
                FinSi
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, ".");
            ...
            6: //"distancia": // distancia 3->ax 4->ay 5->bx 6->by // RAIZ((puntAOrdeY-puntBOrdeY)^2 + (puntACoorX-puntBCoorX)^2)
                ...
                Si (resultat = 5) Entonces
                    nota <- 2;
                FinSi
                cadenaCorreccio <- CONCATENAR("Distancia -> ", CONVERTIRATEXTO(resultat));
            7: //"mitjArit": // mitjana aritmetica
                ...
                Si (resultat = mitArit) Entonces
                    nota <- 2;
                FinSi
                cadenaCorreccio <- "mitjana aritmetica";
        FinSegun
    FinSi
    si ((codiExercici=1) | (codiExercici=2) | (codiExercici=3) | (codiExercici=4)) Entonces
        cadenaCorreccio <- "Sera correcte si ";
        textAuxiliar <- CONVERTIRATEXTO(vValorsEntrats[0]);
        cadenaCorreccio <- CONCATENAR(cadenaCorreccio, textAuxiliar);
        cadenaCorreccio <- CONCATENAR(cadenaCorreccio, " ");
        cadenaCorreccio <- CONCATENAR(cadenaCorreccio, operador);
        cadenaCorreccio <- CONCATENAR(cadenaCorreccio, " ");
        textAuxiliar <- CONVERTIRATEXTO(vValorsEntrats[1]);
        cadenaCorreccio <- CONCATENAR(cadenaCorreccio, textAuxiliar);
        cadenaCorreccio <- CONCATENAR(cadenaCorreccio, " es igual a ");
        textAuxiliar <- CONVERTIRATEXTO(resultat);
        cadenaCorreccio <- CONCATENAR(cadenaCorreccio, textAuxiliar);
        cadenaCorreccio <- CONCATENAR(cadenaCorreccio, ".");
    SiNo
        si (codiExercici=5) Entonces
            cadenaCorreccio <- "Per que sigui correcte cal escriure un missatge del tipus -> No es pot divir per zero! (codi -2001)";
			
        SiNo
            si (codiExercici=6) Entonces
                cadenaCorreccio <- "Per que sigui correcte";
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, " la distancia entre Punt A(");
                textAuxiliar <- CONVERTIRATEXTO(vValorsEntrats[3]);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, textAuxiliar);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, ",");
                textAuxiliar <- CONVERTIRATEXTO(vValorsEntrats[4]);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, textAuxiliar);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, ") i Punt B(");
                textAuxiliar <- CONVERTIRATEXTO(vValorsEntrats[5]);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, textAuxiliar);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, ",");
                textAuxiliar <- CONVERTIRATEXTO(vValorsEntrats[6]);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, textAuxiliar);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, ") ha de ser: ");
                textAuxiliar <- CONVERTIRATEXTO(resultat);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, textAuxiliar);
                cadenaCorreccio <- CONCATENAR(cadenaCorreccio, ".");
            FinSi
