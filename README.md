...
### **Pas 3.** que hi pugeu el fitxer **```<CognomAlumne>-examen-20230120.psc```** amb el codi font en pseudocodi per el que es demana a assolir cadascun dels enunciats.
<br>
...
Escriu en el fitxer **```<CognomAlumne>-examen-20230120.psc```** un **proces** o una **funció** amb el nom **```elTriangleEs```** que rebi els 3 costats d'un triangle com a paràmetres i informi de quin tipus és el triangle es tracta.
...
### Escriu en el fitxer **```<CognomAlumne>-examen-20230120.psc```** un **proces** o una **funció** amb el nom **```llegirPositiuAmbMissatge```** per llegir un nombre enter positiu per teclat. Aquesta funció rebrà un paràmetre de tipus **```Cadena```** que serà el missatge a mostrar a l'usuari, per demanar el nombre enter positiu.	
...
## Del grup de línies des de **Linia 1.1** fins **Linia 1.6**, hi ha alguna línia de codi que es pugui eliminar?
...
Dins del procès **```mostraResultats```** hi ha un bloc **```SI```** que es repeteix dues vegades.

```java
SubProceso mostraResultats(vNotesRebudes Por Referencia, matTextExercicisRebuda, finsLaNota, totalNotes)
...
  Si (finsLaNota = totalNotes) Entonces        //   Linia 1.2
...
  FinSi
...
...
## Hi ha alguna possiblitat de eliminar un dels dos blocs i que segueixi fucionant?
...
## Desenvolupa en el fitxer **```<CognomAlumne>-examen-20230120.psc```** el codi necesari per que el procès **NOMÉS** mostri les notes que es troven entre les dues posicions que li passem **```desDeLaNota```**.

## La defnició quedaria de la següent manera:
```java
